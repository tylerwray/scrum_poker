defmodule ScrumPokerWeb.LiveHelpers do
  ...
  def flash(%{kind: :error} = assigns) do
    ~H"""
    <%= if Phoenix.Component.live_flash(@flash, @kind) do %>
      <div
        id="flash"
      ...
      >
        <div class="flex justify-between items-center space-x-3 text-red-700">
          <.icon name={:exclamation_circle} class="w-5" />
          <p class="flex-1 text-sm font-medium" role="alert">
            <%= Phoenix.Component.live_flash(@flash, @kind) %>
          </p>
          <button
            type="button"
